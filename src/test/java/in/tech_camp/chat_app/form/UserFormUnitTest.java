...
// Validation.buildDefaultValidatorFactory();は、JavaのBean Validation API (JSR 380)を使用してバリデータファクトリーを生成するためのメソッド
  ...
  class ユーザーを作成できない場合 {
    ...
    public void passwordとpasswordConfirmationが不一致では登録できない() {
      ...
      verify(bindingResult).rejectValue("passwordConfirmation", null, "Password confirmation doesn't match Password");
    }
  }
}
